...
用例里的 ``EMBY_PLAYBACK_START`` 是线上抓到的一份**真实**事件（Emby 4.9.5.0 →
MoviePilot 的 ``playback.start``），用来把插件对真实载荷形态的假设钉住：

- 播放位置只出现在 ``PlaybackInfo.PositionTicks``，``Session`` 里**没有**这个字段。
  事件指纹与播放处理都依赖这条回退，任何一处删掉回退都会让去重/进度同步静默退化；
- ``playback.start`` 不是同步触发点（同步发生在 pause / stop），这类事件不得产生同步；
- 该条目 ``ProviderIds`` 为空，即 Emby 侧拿不到 Tmdb 号，匹配只能退回按名称搜索 ——
  这是模糊匹配链路的真实前提。
...
def _payload(
    event_name: str = "playback.start", position_ticks: Optional[int] = None
) -> dict[str, Any]:
    """取一份载荷副本（播放处理会就地改写 ``Session``，必须深拷贝）。"""
    data = copy.deepcopy(EMBY_PLAYBACK_START)
