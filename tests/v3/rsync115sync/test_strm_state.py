...
# --------------------------------------------------------------------------
# dest_probe_outcome —— 探测结论折算（云端可见性）
# --------------------------------------------------------------------------

def test_dest_absent_wins_on_contradictory_input():
    """
    自相矛盾的输入（既说不可见、又读到了大小）必须归为「不可见」，绝不能报「完好」。

    ⚠️ 修注：这里早期写的是「顺序是承重的」，**那句话是错的**。把三个分支任意重排，
    8 种输入组合的行为完全一致（互斥分支，重排属等价变异，改掉也不会让用例失败）。
    保留这条断言是因为「不可见 + 读到大小」在挂载抖动时真会出现，
    而它必须落在一个不安全的方向上 —— 不能是 DEST_OK。
    """
    assert strm.dest_probe_outcome(dest_size=123, src_size=123,
                                   dest_missing=True) == strm.DEST_ABSENT


def test_dest_same_size_is_ok():
    assert strm.dest_probe_outcome(123, 123, False) == strm.DEST_OK


def test_dest_size_mismatch_is_mismatch():
    assert strm.dest_probe_outcome(100, 123, False) == strm.DEST_SIZE_MISMATCH


@pytest.mark.parametrize("dest,src", [(None, 123), (123, None), (None, None)])
def test_unreadable_size_is_unknown_not_absent(dest, src):
    """
    读不到大小 ≠ 文件不存在。

    二者必须分开：挂载点抖动时 `getsize` 抛 OSError，若压成「不存在」，
    用户会被告知「云端没有这个文件」并据此去删旧重传 —— 而文件其实是好的。
    """
    assert strm.dest_probe_outcome(dest, src, False) == strm.DEST_UNKNOWN


def test_dest_path_of_joins_dest_root_with_relative_path():
    pairs = [{"name": "电视剧", "src": "/s/TV", "dest": "/mnt/115/TV",
              "strm_dir": "/strm/TV"}]
    assert strm.dest_path_of("电视剧:a/b.mkv", pairs) == "/mnt/115/TV/a/b.mkv"


def test_dest_path_of_unmapped_and_empty_dest_return_none():
    """无归属映射、或映射没填 dest 时都返回 None —— 探测必须退化成 unknown 而不是猜。"""
    assert strm.dest_path_of("其它剧:a.mkv",
                             [{"name": "电视剧", "dest": "/mnt/115/TV"}]) is None
    assert strm.dest_path_of("电视剧:a.mkv",
                             [{"name": "电视剧", "dest": ""}]) is None
