...
def test_residues_of_this_file_are_removed(tmp_path):
    """正式文件被删后，同目录里属于它的残留一并清掉。"""
    plugin = _plugin(str(tmp_path))
    _write(os.path.join(plugin._sync_pairs[0]["src"], "a.mkv"))
    _write(os.path.join(plugin._sync_pairs[0]["dest"], "a.mkv"))
    dest_dir = plugin._sync_pairs[0]["dest"]
    residue = os.path.join(dest_dir, ".a.mkv.WCTOKM")
    _write(residue)

    deleted, _ = plugin._delete_dest_files_for_retry([KEY])
    assert deleted == [KEY]
    assert not os.path.exists(residue), "残留没被清掉，之后每次探测都会判成有残留"


def test_residue_of_another_file_is_never_touched(tmp_path):
    """
    **只删属于这一个文件的残留。**

    残留判据是「同目录 + 同一正式名 + 短随机后缀」。同目录下别的文件的残留
    （`b.mkv..xrp4gj`）与它毫无关系，误删就是净损失 —— 本条是这条护栏的核心。
    """
    plugin = _plugin(str(tmp_path))
    _write(os.path.join(plugin._sync_pairs[0]["src"], "a.mkv"))
    _write(os.path.join(plugin._sync_pairs[0]["dest"], "a.mkv"))
    dest_dir = plugin._sync_pairs[0]["dest"]
    others = [
        os.path.join(dest_dir, "b.mkv..xrp4gj"),      # 别的正式文件的残留
        os.path.join(dest_dir, "a.srt..xrp4gj"),      # 同主名、别的扩展名
        os.path.join(dest_dir, "other.mkv.WCTOKM"),
    ]
    for p in others:
        _write(p)

    plugin._delete_dest_files_for_retry([KEY])
    for p in others:
        assert os.path.exists(p), f"误删了别的文件的残留: {os.path.basename(p)}"


def test_residue_removed_even_when_official_file_absent(tmp_path):
    """
    正式文件不存在、只剩残留 —— 这是改名失败的典型形态，残留同样要清。

    原先的实现只在「目标端有正式文件」的分支里走到残留清理，于是这个**最主要的
    场景**反而漏清：正式名不存在 → 直接 continue → 残留永久留在云端。
    """
    plugin = _plugin(str(tmp_path))
    _write(os.path.join(plugin._sync_pairs[0]["src"], "a.mkv"))
    residue = os.path.join(plugin._sync_pairs[0]["dest"], ".a.mkv.WCTOKM")
    _write(residue)                                  # 只有残留，没有正式文件
