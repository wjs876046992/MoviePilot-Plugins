...
# 默认 **30 分钟**一轮（2026-09-25 从 `*/10` 调宽）。
# 理由：每轮是每个映射一次全树 `os.walk`，大媒体库下这个成本是实打实的；
# 而"发现得早一点"对最终上传时间几乎没有影响 —— 文件进队列后还要等满冷却
# （默认 4 小时），扫描频率只决定"它多早**进入**那 4 小时的等待"。
# 30 分钟对媒体入库的时效已经足够，把成本省下来。
SOURCE_SCAN_CRON_DEFAULT = "*/30 * * * *"
