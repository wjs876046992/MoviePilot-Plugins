...
- [ ] 确认这些文件留在疑似清单并显示「已请求生成」标记（v0.1.9 起不再移入观察期）
- [ ] 确认观察期条目的「检查 strm」按钮能即时判定（已生成 ⇒ 立即解除；未到期 ⇒ 继续等待）
- [ ] 确认「补生成无效」标记只在宽限期到期后仍无 strm 时出现
- [ ] 若助手日志报「路径匹配错误」→ 说明第 1 步的助手配置没对上，需记录并补充提示文案

### [ ] 14. 补生成的进度可见性（观察项）

助手是异步长任务，插件侧拿不到完成回调，当前只能靠「宽限期到期后看结果」。
若用户反馈「点了之后不知道在跑还是没跑」，可考虑：
...
**已完成**（alpha `7a60ee3`）：`tests/v3/rsync115sync/` 共 212 项，覆盖核心同步
